

# This file was *autogenerated* from the file comp_iguais_bgv-like.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_448 = Integer(448); _sage_const_64 = Integer(64); _sage_const_8 = Integer(8)
load("dghv_bgv-like.sage")

def compIguais(dghv, n=_sage_const_1 ):
    m0 = ZZ.random_element(_sage_const_0 , _sage_const_2 **n)
    m1 = ZZ.random_element(_sage_const_0 , _sage_const_2 **n)

    print(f"m0 = {m0}")
    print(f"m1 = {m1}")

    bits0 = m0.digits(base=_sage_const_2 , padto=n)
    bits1 = m1.digits(base=_sage_const_2 , padto=n)

    c0 = [dghv.enc(bi) for bi in bits0]
    c1 = [dghv.enc(bi) for bi in bits1]
        # Codifica os bits na forma vetorial

    c = dghv.enc(_sage_const_1 )
        # Codifica c 
    const1 = dghv.enc(_sage_const_1 )
        # Codifica constante 1 para realizar a porta not (c XOR 1)
    for i in range(n):
        cmp_i = dghv.add(c0[i], c1[i])       # c0[i] XOR c1[i]
        cmp_i = dghv.not_gate(cmp_i)
        c = dghv.mult(c, cmp_i)              # c = c AND cmp_i

    res = dghv.dec(c)
    print(f"Esperado: {_sage_const_1  if m0==m1 else _sage_const_0 } | Res: {res}")
    
dghv = DGHV(_sage_const_448 , _sage_const_64 , _sage_const_8 )
compIguais(dghv)

# fresco: sk = p e log(ruído) ~= rho
# depois de multiplicar: sk = p' e log(ruído) ~= rho
# os criptogramas podem estar criptografados com ps diferentes
# p0 > p1 > ... > pL
# modulo switching pro nivel certo
# rho = lambda, eta = maior que lambda, gamma = lambda * (eta - rho)^2 / log(lambda) (pensando em segurança)
# lamda -> 2^lambda operações -> lambda é o parâmetro de segurança
# escolha de eta -> baseado no tamanho do primo que depende do circuito
# pra brincar -> lambda pequeno

